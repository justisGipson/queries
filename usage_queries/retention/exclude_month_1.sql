...
SELECT
  cohort_month,
  COALESCE(ROUND(SUM(CASE WHEN week_number = 1 THEN active_users END) * 1.0 / MAX(cohort_users) * 100, 2),0) AS week_1_retention,
  COALESCE(ROUND(SUM(CASE WHEN week_number = 2 THEN active_users END) * 1.0 / MAX(cohort_users) * 100, 2),0) AS week_2_retention,
  COALESCE(ROUND(SUM(CASE WHEN week_number = 3 THEN active_users END) * 1.0 / MAX(cohort_users) * 100, 2),0) AS week_3_retention,
  COALESCE(ROUND(SUM(CASE WHEN week_number = 4 THEN active_users END) * 1.0 / MAX(cohort_users) * 100, 2),0) AS week_4_retention,
  COALESCE(ROUND(SUM(CASE WHEN week_number = 5 THEN active_users END) * 1.0 / MAX(cohort_users) * 100, 2),0) AS week_5_retention
