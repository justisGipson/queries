-- Maybe a bit better
-- Calculate User Engagement Frequency (UEF) for each user from activity table
-- no emails, userids instead
...
SELECT
    ...
    to_char(ROUND(AVG(au.total_activity_days * 1.0 / (SELECT total_days FROM total_days) * 100), 2), '999D99%') AS "Year to Date"
FROM active_users au
